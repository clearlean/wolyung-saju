 ...
 *
 * `?format=csv` 로 부르면 전체를 CSV 로 내려보낸다. 배포처(OpenAI Sites)의 D1
 * 에는 wrangler 로 붙을 수 없어서, 이 내보내기가 백업이자 매칭 작업용 자료를
 * 빼내는 유일한 수단이다.
 */
...
SELECT instagram, name, birth_date, calendar_type, birth_time, hour_known,
       gender, year_pillar, month_pillar, day_pillar, hour_pillar,
